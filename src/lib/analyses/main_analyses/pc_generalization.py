...
def _pc_generalization(
    ...
    def _plots(dfs, subject, subject_prefix=None):
        ...
        if reorder_by_cluster:
            _plot_generalization(dfs, 'pc', 'ev', generalization_dim, cache_path, f"fixedby1_{subject_prefix}{subject}", reorder_by_cluster=reorder_by_cluster, n_row_lim=None)
            _plot_generalization(dfs, 'pc', 'r', generalization_dim, cache_path, f"fixedby1_{subject_prefix}{subject}", reorder_by_cluster=reorder_by_cluster, n_row_lim=None)
        _plot_generalization(dfs, 'pc', 'ev', generalization_dim, cache_path, f"{subject_prefix}{subject}", first_row_order_only=False, reorder_by_cluster=reorder_by_cluster, n_row_lim=None)
        _plot_generalization(dfs, 'pc', 'r', generalization_dim, cache_path, f"{subject_prefix}{subject}", first_row_order_only=False, reorder_by_cluster=reorder_by_cluster, n_row_lim=None)
    ...
    for subject in tqdm(X.subject.values, desc="subject"):
        ...
        for split_pair in itertools.permutations(x.split.values, 2) if split_dim else [None]:
            ...
            for g_train in x[generalization_dim]:
                ...
                # x_train_fit = torch.tensor(x.sel({generalization_dim: g_train, "split": split_pair[0]}).values if split_dim else x.sel({generalization_dim: g_train}).values)
                pca.fit(x_train_fit)
                z_train_split0 = pca.transform(x_train_fit)
                
                x_test_split1 = x.sel({"split": split_pair[1]}) if split_dim else x.sel({generalization_dim: g_test})
                ...
                )).reshape([x_test_split1.shape[2], z_train_split0.shape[0], z_train_split0.shape[1]])
                r = pearson_r(z_train_split0, z_test_split1).cpu().numpy()
                cvev = covariance(z_train_split0, z_test_split1).cpu().numpy()
...
def pc_generalization(
    ...
    **kwargs,
    ...
    temp_cache_path = f"main_analyses/pc_generalization/analysis={analysis}/{generalization_id}"
    ...
    match space:
        ...
        case "eeg":
            ...
            # if pc_dims[0] == "time":
            times = load_significant_times(analysis, dataset, load_dataset_kwargs, scorer_kwargs, feature_dim, alpha, subjects="intersection")
            X = X.sel(time=[t in times for t in X.time.values])
            
    reorder_by_cluster = generalization_dim != "time" and analysis != "bhvpc" and analysis != "rn50"
