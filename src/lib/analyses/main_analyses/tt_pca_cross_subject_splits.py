...
def _tt_pca_cross_subject_splits(
    ...
    for subject0, subject1 in tqdm(itertools.permutations(X_train.subject.values, 2), desc="subjects"):
        ...
        x_test0 = X_test.sel(subject=subject0).dropna("time")
        x_test1 = X_test.sel(subject=subject1).dropna("time")
        x_test0 = x_test0.transpose("split", *pc_dims, generalization_dim) if split_dim else x_test0.transpose(*pc_dims, generalization_dim)
        x_test1 = x_test1.transpose("split", *pc_dims, generalization_dim) if split_dim else x_test1.transpose(*pc_dims, generalization_dim)
        ...
        for i, g_train in enumerate(x_train[generalization_dim].values):
            ...
            for split_pair in itertools.permutations(x_test[0].split.values, 2) if split_dim else [None]:
                for subject_split_pair in itertools.permutations(x_test[0].split.values, 2) if split_dim else [None]:
                    x_test_split0 = torch.tensor(x_test[subject_split_pair[0]].sel({"split": split_pair[0], generalization_dim: g_train}).values if split_dim else x_test[subject_split_pair[0]].sel({generalization_dim: g_train}).values).to("cuda")
                    ...
                    x_test_split1 = x_test[subject_split_pair[1]].sel({"split": split_pair[1]}) if split_dim else x_test[subject_split_pair[1]]
...
def tt_pca_cross_subject_splits(
    dataset: str,
    ...
    **kwargs,
    ...
    temp_cache_path = f"main_analyses/tt_pca_cross_subject_splits/analysis={analysis}/{generalization_id}"
    ...
    return cacher(_tt_pca_cross_subject_splits)(X_train, X_test, cache_path, pc_dims, generalization_dim, split_dim=split_dim)