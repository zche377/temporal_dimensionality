...
def _plot_umap(X, dataset, file_path, batch_size=32):
    ...
    for batch_start in tqdm(range(0, len(X), batch_size), desc="batch"):
        ...
        #             img.paste(blurred_face, (x1, y1))
            ...
            image_box = OffsetImage(stim_val, zoom=0.07)
...
def _all_var(
    ...
    for subject in range(1, n_subject+1):
        ...
        distance = pdistance(y_remain.T, return_diagonal=False)
...
def _behavior_remaining_var(
    ...
    for subject in range(1, n_subject+1):
        ...
        distance = pdistance(y_remain.T, return_diagonal=False)
        distance.fill_diagonal_(0)  # Ensure diagonal is zero
        # distance = euclidean_distance(y_remain.T, return_diagonal=False)
...
def remaining_var(
    ...
    cache_str = f"main_analyses/remaining_var_pdistance.pca={pca}.test_images={test_images}/regress_out={analysis}"
    # cache_str = f"main_analyses/{analysis}_remaining_var_mds.pca={pca}.test_images={test_images}"
    ...
    match analysis:
        case "none":
            ...
            cache_kwargs = {}
        case "behavior":
            analysis_fn = _behavior_remaining_var
            cache_kwargs = {}
        case "model_srpz":
            assert "model_uid" in list(model_kwargs.keys())
            analysis_fn = _model_srp_remaining_var
