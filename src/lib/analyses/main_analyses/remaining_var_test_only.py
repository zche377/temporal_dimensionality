...
def _behavior_remaining_var_test_only(
    dataset: str,
    load_dataset_kwargs: dict,
    n_subject: int,
    target_var: str,
    ...
    for subject in range(1, n_subject+1):
        ...
        lmodel = cacher(scorer._fit_model)(
            x_train=_2t(target_test), y_train=_2t(subject_data_test)
        )
...
def _model_srp_remaining_var_test_only(
    dataset: str,
...
def remaining_var_test_only(
    ...
    cache_str = f"main_analyses/remaining_var_test_only.pca={pca}.test_images={test_images}/regress_out={analysis}"
    # cache_str = f"main_analyses/{analysis}_remaining_var_test_only_mds.pca={pca}.test_images={test_images}"
    ...
    predictor_cache_path = f"main_analyses/{analysis}_tt_encoding_test_only.subset=False.pca={pca}/dataset={dataset}"
    ...
    match analysis:
        case "none":
            ...
            cache_kwargs = {}
        case "behavior":
            analysis_fn = _behavior_remaining_var_test_only
            cache_kwargs = {}
        case "model_srpz":
            assert "model_uid" in list(model_kwargs.keys())
            analysis_fn = _model_srp_remaining_var_test_only
