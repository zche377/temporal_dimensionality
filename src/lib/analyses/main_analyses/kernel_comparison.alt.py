...
def _compute_kernels(x, comparison_dim, split=None):
    ...
    for c in x[comparison_dim]:
        if split is not None:
            xt = torch.tensor(x.sel({comparison_dim: c, "split": split}).values).T.to("cuda")
        else:
            xt = torch.tensor(x.sel({comparison_dim: c}).values).T.to("cuda")
        if random_indices is None:
            random_indices = torch.argsort(torch.rand(xt.size(), generator=rng_pt), dim=1).to("cuda")
        xt = xt.gather(1, random_indices)
        kernels.append(_flattend_tril(1 - pearson_r(xt, return_diagonal=False)))
...
def _kernel_comparison(X, kernel_dims, comparison_dim, cache_path, reorder_by_cluster, split_dim=None):
    ...
    for subject in X.subject.values:
        ...
        subject_null_kc = np.concatenate(subject_null_kc, axis=0)
    ...
    null_kc = np.concatenate(null_kcs, axis=0)
...
def kernel_comparison(
    ...
    **kwargs,
    ...
    temp_cache_path = f"main_analyses/kernel_comparison.shuffle_pcdim_same_across_gendim/analysis={analysis}/{comparison_id}"
