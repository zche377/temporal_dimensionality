...
def _split_dim_weights(analysis, dataset, load_dataset_kwargs, scorer_kwargs, split_dim, n_splits, n_seeds, target_var,):
    ...
    for subject in weights.subject.values:
        ...
        for seed in weights.seed.values:
            ...
            for t in weights.time.values:
                ...
                pair_scores = []
                dims = (f"{target_var}_split_i", f"{target_var}_split_j")
...
def split_dim_weights(
    ...
    temp_cache_path = f"weights_sanity_checks/split_dim_weights/{analysis}_decoding/{split_id}"
    ...
    return cacher(_split_dim_weights)(
